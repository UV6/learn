...
> **架构定位**：本章继续实现 ④ 记忆层，焦点从会话内的上下文压缩转向跨会话的持久记忆。
...
```text
1. 项目根目录/CODIA.md          <- 项目级（最高优先级）
2. 项目根目录/.codia/CODIA.md <- 项目级（可被 .gitignore 忽略）
3. ~/.codia/CODIA.md           <- 用户级（个人偏好）
```
...
**第一步：逐行解析 JSONL**。跟普通 JSON 的一次性解析不同，JSONL 可以逐行独立读取，遇到解析失败的行就跳过继续：
...
**第二步：验证消息链完整性**。JSONL 文件可能因为崩溃而截断，比如有一条 assistant 消息包含了工具调用请求，但对应的 `tool_result` 还没来得及写入就崩溃了。模型看到一个工具调用但没有结果，会很困惑。
...
**第三步：检查 token 量**。如果恢复的会话很长，token 量可能已经超过压缩阈值。直接触发一次压缩再返回给用户，免得用户刚恢复会话就因为 token 超限报错。

**第四步：插入时间跨度提示**。如果距离上次活跃超过 24 小时，在对话中插入一条消息提醒 Agent：上次会话是什么时候，中间可能有代码变更，建议重新读取相关文件。这能有效避免 Agent 拿着过期的文件内容做决策。
