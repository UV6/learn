...
![System Architecture|697](images/system-architecture.svg)
...
| 组件                              | 位置                  | 作用                                                             |
| ------------------------------- | ------------------- | -------------------------------------------------------------- |
| `UserPromptSubmit` hooks        | 用户输入前后              | 记录、注入、审计用户输入                                                   |
| cron queue                      | LLM 前               | 把定时触发的 prompt 注入 `messages`                                    |
| background notifications        | LLM 前               | 后台任务完成后以 `<task_notification>` 注入                              |
| compaction pipeline             | LLM 前               | 先压大输出，再裁历史，再压旧 tool_result，必要时摘要                               |
| memory / skills / MCP state     | LLM 前               | 组装 system prompt，让模型看到当前能力和长期上下文                               |
| error recovery                  | LLM 调用              | 429/529 重试，`max_tokens` 升级，prompt too long 触发 reactive compact |
| `PreToolUse` hooks + permission | 工具执行前               | 拦截危险命令、写越界、破坏性 MCP 工具                                          |
| `assemble_tool_pool`            | 工具分发                | 组装内置工具和 MCP 动态工具                                               |
| background dispatch             | 工具执行时               | 慢 bash 操作放 daemon thread，主循环先返回占位结果                            |
| `PostToolUse` hooks             | 工具执行后               | 大输出告警、日志等后处理                                                   |
| tool_result                     | 返回循环                | 每个 `tool_use` 对应一个 `tool_result`，再回到下一轮                        |
| `Stop` hooks                    | 本轮没有 tool_use / 停止时 | 统计、清理、审计                                                       |
